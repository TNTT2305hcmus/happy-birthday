...
$tempRoot = Join-Path ([IO.Path]::GetTempPath()) ('twinkle-r46-' + [guid]::NewGuid().ToString('N'))
$server = $null
$edge = $null
$port = 9352
...
try {
  ...
  $server = Start-Process -FilePath 'node.exe' -ArgumentList @($vitePath, '--host', '127.0.0.1', '--port', '4176') -WorkingDirectory $projectRoot -WindowStyle Hidden -PassThru
  $serverReady = $false
  ...
  & node.exe (Join-Path $PSScriptRoot 'verify-refinement-r4-6-cdp.mjs') $port $origin
  if ($LASTEXITCODE -ne 0) { throw 'R4.6 browser verification failed.' }
} finally {
