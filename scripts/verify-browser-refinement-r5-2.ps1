...
$tempRoot = Join-Path ([IO.Path]::GetTempPath()) ('twinkle-r52-' + [guid]::NewGuid().ToString('N'))
$server = $null
$edge = $null
$port = 9354
...
try {
  ...
  $server = Start-Process -FilePath 'node.exe' -ArgumentList @($vitePath, '--host', '127.0.0.1', '--port', '4178') -WorkingDirectory $projectRoot -WindowStyle Hidden -PassThru
  $serverReady = $false
  ...
  & node.exe (Join-Path $PSScriptRoot 'verify-refinement-r5-2-cdp.mjs') $port $origin
  if ($LASTEXITCODE -ne 0) { throw 'R5.2 browser verification failed.' }
} finally {
