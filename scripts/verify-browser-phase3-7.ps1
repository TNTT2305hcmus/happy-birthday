...
$tempRoot = Join-Path ([IO.Path]::GetTempPath()) ('twinkle-phase37-' + [guid]::NewGuid().ToString('N'))
$server = $null
$edge = $null
$port = 9347
...
try {
  ...
  $readerPath = Join-Path $PSScriptRoot 'verify-phase3-7-browser-cdp.mjs'
  & node.exe $readerPath $port $origin
  if ($LASTEXITCODE -ne 0) {
    throw 'Phase 3.7 browser verification failed.'
  }
} finally {
