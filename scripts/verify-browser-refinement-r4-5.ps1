...
$tempRoot = Join-Path ([IO.Path]::GetTempPath()) ('twinkle-r45-' + [guid]::NewGuid().ToString('N'))
$server = $null
$edge = $null
$port = 9350
...
try {
  ...
  $server = Start-Process -FilePath 'node.exe' -ArgumentList @($vitePath, '--host', '127.0.0.1', '--port', '4174') -WorkingDirectory $projectRoot -WindowStyle Hidden -PassThru
  $serverReady = $false
  ...
  if (-not $serverReady) { throw 'Vite did not become ready on port 4174.' }
  ...
  & node.exe (Join-Path $PSScriptRoot 'verify-refinement-r4-5-cdp.mjs') $port $origin
  if ($LASTEXITCODE -ne 0) { throw 'R4.5 browser verification failed.' }
} finally {
